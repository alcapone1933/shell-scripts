...
function print_help {
    ...
    echo ""
    echo "  $program_name install script"
    echo ""
    ...
    echo ""
    echo "  for linux os   : linux/386 linux/amd64 linux/arm64"
}
# Check if the program is already installed
function check_installed {
    if [ -x "$(command -v $program_cmd)" ]; then
        $program_cmd --version
        return 0
    else
        return 1
...
function update {
    ...
    current_version=$($program_cmd --version)
    latest_version=$(curl -s https://api.github.com/repos/${program_repo}/releases/latest | grep 'tag_name' | cut -d\" -f4)
    ...
    # Print the updated version
    $program_cmd --version
    echo "Program $program_name is Updating from v$current_version to $latest_version version ..."
}
...
function remove {
    ...
    rm -v /usr/local/bin/$program_cmd
