...
program_name="pueue"
program_named="pueued"
program_repo="Nukesor/pueue"
...
function check_and_install_dependencies {
    if [ -f /etc/debian_version ]; then
        ...
        if ! dpkg -l | grep -q "libgcc1"; then
            echo "libgcc1 is not installed. Installing libgcc1..."
            apt update && apt install -y libgcc1 || { echo "Error installing libgcc1."; exit 1; }
        else
            echo "libgcc1 is already installed."
        fi
    elif [ -f /etc/alpine-release ]; then
        # Alpine Linux System
        if ! apk info | grep -q "libgcc"; then
            echo "libgcc is not installed. Installing libgcc..."
            apk add --no-cache libgcc || { echo "Error installing libgcc."; exit 1; }
        else
            echo "libgcc is already installed."
        fi
    else
...
function install {
    ...
    case $(uname -m) in
        x86_64|amd64)
            ...
            ;;
        armv7l|armhf)
            os="armv7-unknown-linux-musleabihf"
            ;;
        aarch64|arm64)
    ...
    curl -L# "$program_download_url/$latest_version/${program_named}-$os" -o /usr/local/bin/$program_named

    chmod +x /usr/local/bin/$program_name /usr/local/bin/$program_named
    # Print the updated version
    echo "$program_name --version"
    $program_name --version
    echo "$program_named --version"
    $program_named --version
}
...
function update {
    ...
    current_version=$($program_name --version | cut -d' ' -f2)
    latest_version=$(curl -s https://api.github.com/repos/${program_repo}/releases/latest | grep 'tag_name' | cut -d\" -f4)
...
function remove {
    ...
    echo "Removing $program_name and $program_named ..."
    rm -v /usr/local/bin/$program_name /usr/local/bin/$program_named
    if check_installed; then
